...
pub fn ves_test_suite(_: TokenStream, item: TokenStream) -> TokenStream {
    let mut module =
        syn::parse::<syn::ItemMod>(item).expect("This macro must be used on a module.");
        ...
        match check_attrs(item) {
            Ok(Some(attr)) => match attr {
                ...
                SuiteItemKind::TestsNamespace(path) => {
                    let namespace = match &path {
                        ...
                        _ => {
                            return syn::Error::new(
                                path.span(),
                                "The path literal must be a string",
                            )
                            .into_compile_error()
                            .into()
                        }
                    ...
                    let tests = match std::fs::read_dir(tests_dir) {
                        ...
                        Err(e) => {
                            return syn::Error::new(
                                path.span(),
                                format!("Failed to read the test files: {}", e),
                            )
                            .into_compile_error()
                            .into();
                        }
                    ...
                    if callbacks.err.is_some() {
                        ...
                        if callbacks.input.is_some() {
                            ...
                            append!(content,
                                ves_testing::make_test_macros!(__gen_CRATE_ROOT, __gen_TESTS_DIR, #ok, #err, #input);
                            );
                        } else {
                            append!(content,
                                ves_testing::make_test_macros!(__gen_CRATE_ROOT, __gen_TESTS_DIR, #ok, #err);
                            );
                        }
                    } else if callbacks.input.is_some() {
                        let input = callbacks.input.unwrap();
                        append!(content,
                            ves_testing::make_test_macros!(eq => __gen_CRATE_ROOT, __gen_TESTS_DIR, #ok, #input);
                        );
                    } else {
                        append!(content,
                            ves_testing::make_test_macros!(eq => __gen_CRATE_ROOT, __gen_TESTS_DIR, #ok);
                        );
                    }
                ...
                _ => {
                    return syn::Error::new(
                        item.span(),
                        "The suite items may only have the ves_tests and ves_test_root attributes.",
                    )
                    .into_compile_error()
                    .into()
                }
...
fn check_attrs(item: &mut syn::Item) -> Result<Option<SuiteItemKind>, syn::Error> {
    Ok(match item {
        syn::Item::Static(st) => match has_attr(&st.attrs, "ves_suite_root") {
            Some(idx) => {
                st.attrs.remove(idx);
        ...
        syn::Item::Mod(module) => match has_attr(&module.attrs, "ves_tests") {
            Some(idx) => {
        ...
        syn::Item::Use(r#use) => {
            ...
            if let Some(idx) = has_attr(&r#use.attrs, "ok_callback")
                .or_else(|| has_attr(&r#use.attrs, "eq_callback"))
            {
        ...
        syn::Item::Fn(r#fn) => {
            if let Some(idx) = has_attr(&r#fn.attrs, "ok_callback")
                .or_else(|| has_attr(&r#fn.attrs, "eq_callback"))
            {
