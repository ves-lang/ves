...
pub type Scope<K, T> = AHashMap<K, T>;

/// A scoped environment that uses a [`Vec`] of hashmaps internally.
/// Note that the hashing algorithm used ia `ahash`, which is quick but less
    ...
    K: std::hash::Hash + Eq,
{
    ...
    pub fn new() -> Self {
        Self {
            // Assume 5-level nesting with 32 reachable variables on average.
            scopes: {
